import { logger } from "~/logging/logging";
type AnalyticsTagProps = {
  sporingId: string;
  sporingScriptUrl: string;
};

export function AnalyticsTags({ sporingId, sporingScriptUrl }: AnalyticsTagProps) {
  return (
    <script
      defer
      src={sporingScriptUrl}
      data-host-url="https://umami.nav.no"
      data-website-id={sporingId}
    />
  );
}

/** Spor en hendelse til analyseformål
 *
 * Du kan sende med et objekt med relevante data for hendelsen.
 */
export function sporHendelse(hendelse: Hendelse, data: Record<string, unknown> = {}) {
  if (process.env.NODE_ENV === "development") {
    if (hendelse.length > 50) {
      logger.warn(
        `📊 [Analytics] Hendelse ${hendelse} er for lang. Maks lengde er 50 tegn, hendelsen er på ${hendelse.length} tegn.`,
      );
    }
    logger.info(`📊 [Analytics] ${hendelse}`, data);
    return;
  }
  if (typeof window !== "undefined" && window.umami) {
    window.umami.track(hendelse.substring(0, 50), data); // Maks lengde er 50 tegn for Umami
  }
}

type Hendelse =
  | "endre tema"
  | "side lastet på nytt grunnet ny versjon"
  | "sak opprettet"
  | "sak status endret"
  | "sak satt på vent"
  | "sak gjenopptatt"
  | "sak redigert"
  | "fordeling utført"
  | "søk utført"
  | "søk resultat valgt"
  | "person oppslag"
  | "sak åpnet"
  | "filter brukt"
  | "journalpost opprettet"
  | "oppgave opprettet"
  | "fil lastet opp"
  | "vedlegg lastet opp"
  | "vedlegg åpnet"
  | "vedlegg lastet ned"
  | "vedlegg omdøpt"
  | "vedlegg slettet"
  | "dokument opprettet"
  | "dokument formatert"
  | "dokument lastet ned"
  | "dokument sletting påbegynt"
  | "dokument slettet"
  | "dokument sletting avbrutt"
  | "kommentarpanel åpnet"
  | "kommentar oppretting startet"
  | "kommentar oppretting avbrutt"
  | "kommentar opprettet"
  | "kommentar oppretting feilet"
  | "kommentarsvar opprettet"
  | "kommentarsvar feilet"
  | "kommentar redigert"
  | "kommentar redigering feilet"
  | "kommentar slettet"
  | "kommentar sletting feilet"
  | "kommentartråd løst"
  | "kommentartråd gjenåpnet"
  | "kommentartråd status feilet"
  | "kommentar ankernavigasjon"
  | "kommentarfilter brukt"
  | "innstillinger endret"
  | "navigere"
  | "varsler åpnet"
  | "varsel markert som lest"
  | "varsler markert som lest"
  | "hurtigsøk aktivert"
  | "opprett sak bekreftelse vist"
  | "opprett sak bekreftet"
  | "opprett sak avbrutt i bekreftelse"
  | "opprett sak skjema nullstilt"
  | "endre status dialog åpnet"
  | "endre status saksstatus valgt"
  | "endre status arbeidsstatus valgt"
  | "endre status bekreftelse vist"
  | "endre status avbrutt i bekreftelse"
  | "endre status lagre klikket"
  | "endre status lagret"
  | "endre status lagring feilet"
  | "endre status dialog avbrutt"
  | "send til annen enhet dialog åpnet"
  | "send til annen enhet bekreftelse vist"
  | "send til annen enhet avbrutt i bekreftelse"
  | "send til annen enhet klikket"
  | "send til annen enhet sendt"
  | "send til annen enhet sending feilet"
  | "journalpost bekreftelse vist"
  | "journalpost avbrutt i bekreftelse"
  | "oppgave bekreftelse vist"
  | "oppgave avbrutt i bekreftelse"
  | "søk opprett sak klikket"
  | "søk watson søk klikket"
  | "søk se alle saker klikket"
  | "migrering opprett sak klikket";
